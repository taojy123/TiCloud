...
class ConsumerRegisterApply(ApplyMixin, models.Model):
    name = models.CharField(max_length=64, blank=True, verbose_name='用户名称', help_text='一般是中文')
    username = models.CharField(max_length=64, blank=True, verbose_name='用户账号', help_text='唯一标识对该用户授权访问的一个访问通道编码。编码规则：机构中文全拼缩写（小写 ）+ 4位16进制随机编码，由业务输入系统 （系统应具备验证用户名唯一性功能）。用户账号由业务生成，生成的过程中，能够校验账号的唯一性')
    ...
    org_number = models.CharField(max_length=128, blank=True, verbose_name='社会统一信用代码/组织机构代码', help_text='必填，用于客户业务校核，客户的中英文名称应该与社会统一信用代码一致')
...
class ConsumerOrderApply(ApplyMixin, models.Model):
...
class ConsumerTrialApply(ApplyMixin, models.Model):
    username = models.CharField(max_length=64, blank=True, verbose_name='用户账号', help_text='唯一标识对该用户授权访问的一个访问通道编码。编码规则：机构中文全拼缩写（小写 ）+ 4位16进制随机编码，由业务输入系统 （系统应具备验证用户名唯一性功能）。用户账号由业务生成，生成的过程中，能够校验账号的唯一性')
...
class VendorApply(ApplyMixin, models.Model):
...
class VendorApiApply(ApplyMixin, models.Model):
    org_code = models.CharField(max_length=64, blank=True, verbose_name='机构编码', help_text='供应商的唯一标识。编码规则：机构中文全拼缩写（小写 ）+ 4位16进制随机编码，由业务输入系统 （系统应具备验证用户名唯一性功能）。用户账号由业务生成，生成的过程中，能够校验账号的唯一性。该账号也是供应商访问日志的登录账号')
...
class ProductLaunchApply(ApplyMixin, models.Model):
...
class Ticket(models.Model):
    ...
    applicant = models.ForeignKey(User, blank=True, null=True, on_delete=models.SET_NULL, related_name='applicant_ticket_set', verbose_name='申请人')  # 姓名 手机 邮箱 部门 职位
    maintainer = models.ForeignKey(User, blank=True, null=True, on_delete=models.SET_NULL, related_name='maintainer_ticket_set', verbose_name='关系维护人')
    ...
    status = models.CharField(max_length=32, blank=True, verbose_name='工单状态', help_text='审批中/通过/驳回/撤回/已生效/归档')
...
class TicketFlow(models.Model):
    ...
    result = models.CharField(max_length=32, blank=True, verbose_name='审批结果', help_text='通过/驳回')
