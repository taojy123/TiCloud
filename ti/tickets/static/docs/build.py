...
def new_restful_apis(res_name, uri, view_set, localhost=LOCALHOST):
    ...
    # ======= GET Detail=======
    api3 = Api()
    api3.title = res_name + '详情'
    api3.uri = uri + '<id>/'
    api3.method = 'GET'
    api3.uri_params = [UP(name='id', description=f'{res_name} ID', example=1)]
    ...
    if data['results']:
        ...
        api3.response_example = json.dumps(data2, ensure_ascii=False, indent=4)
...
api1, api2, api3 = new_restful_apis('第三方供应商注册申请', '/api/tickets/vendor_apply/', VendorApplyViewSet)
doc.apis.append(api2)
doc.apis.append(api3)

api1, api2, api3 = new_restful_apis('第三方API接入申请', '/api/tickets/vendor_api_apply/', VendorApiApplyViewSet)
doc.apis.append(api2)
doc.apis.append(api3)

api1, api2, api3 = new_restful_apis('产品上线申请', '/api/tickets/product_launch_apply/', ProductLaunchApplyViewSet)
doc.apis.append(api2)
doc.apis.append(api3)
