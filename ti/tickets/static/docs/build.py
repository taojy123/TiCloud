...
from tickets.views import ProductLaunchApplyViewSet
...
api2.body_params = [
    BP(name='username',
       description='用户账号(唯一标识对该用户授权访问的一个访问通道编码。编码规则：机构中文全拼缩写（小写 ）+ 4位16进制随机编码，由业务输入系统 （系统应具备验证用户名唯一性功能）。用户账号由业务生成，生成的过程中，能够校验账号的唯一性)',
       required=True),
    BP(name='oa_number',
       description='协议OA审批编号(用户申请测试，如果测试连接第三方生产环境，有费用发生，需要走OA流程，知会财务；如果是自己数据源，小宇100条免费，可以不用走OA，大于100条需要走OA流程审批)',
       required=False),
...
api6.body_params = [
    BP(name='org_code',
       description='机构编码(供应商的唯一标识。编码规则：机构中文全拼缩写（小写 ）+ 4位16进制随机编码，由业务输入系统 （系统应具备验证用户名唯一性功能）。用户账号由业务生成，生成的过程中，能够校验账号的唯一性。该账号也是供应商访问日志的登录账号)',
       required=True),
...
api8.body_params = [
    BP(name='org_code',
       description='机构账编码(供应商的唯一标识。编码规则：机构中文全拼缩写（小写 ）+ 4位16进制随机编码，由业务输入系统 （系统应具备验证用户名唯一性功能）。用户账号由业务生成，生成的过程中，能够校验账号的唯一性。该账号也是供应商访问日志的登录账号)',
       required=True),
...
api9 = Api()
api9.title = '产品上线申请列表'
api9.uri = '/api/tickets/product_launch_apply/'
api9.method = 'GET'
view_set = ProductLaunchApplyViewSet
...
for field_name, kinds in filter_fields.items():
    for kind in kinds:
        ...
        api9.query_params.append(QP(name=field_name, description=description))

url = LOCALHOST + api9.uri
data = requests.get(url).json()
if len(data['results']) > 2:
    data['results'] = [data['results'][0]]
api9.response_example = json.dumps(data, ensure_ascii=False, indent=4)

doc.apis.append(api9)


api10 = Api()
api10.title = '创建产品上线申请'
api10.uri = '/api/tickets/product_launch_apply/'
api10.method = 'POST'
view_set = ProductLaunchApplyViewSet
...
for field_name, field in serializer.fields.items():
    ...
    api10.body_params.append(BP(name=field_name, type=type, description=description, required=required, default=default))

api10.response_example = r"""
{
    "id": 1,
    "product_number": "test222",
    "product_name": "fasfa",
    "source_numbers": "fasdf",
    "manager_name": "张三",
    "manager_mobile": "13402110752",
    "manager_email": "tss@tq.com",
    "use_cache": true,
    "cache_ms": 222,
    "timeout_ms": 344,
    "alarm_total_count": 32,
    "alarm_fail_count": 22,
    "alarm_timeout_count": 2,
    "params_in": "",
    "params_in_sensitive": "",
    "params_out_sensitive": "",
    "creator": "ff",
